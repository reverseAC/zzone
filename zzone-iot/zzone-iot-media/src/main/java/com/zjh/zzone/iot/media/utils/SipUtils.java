...
public class SipUtils {
    ...
    public static  String getNewViaTag() {
        return "z9hG4bK" + RandomStringUtils.randomNumeric(10); // z9hG4bK（via branch固定前缀） + 10位随机数
    }
    ...
    public static String cmdString(int leftRight, int upDown, int inOut, int moveSpeed, int zoomSpeed) {
        ...
        StringBuilder builder = new StringBuilder("A50F01"); // A5:起始码 0F:长度 01:设备地址
        String strTmp;
    ...
    public static RemoteAddressInfo getRemoteAddressFromRequest(SIPRequest request, boolean sipUseSourceIpAsRemoteAddress) {
        ...
        if (sipUseSourceIpAsRemoteAddress) {
            remoteAddress = request.getPeerPacketSourceAddress().getHostAddress(); // 发送方的真实ip（来自网络层），与报文中的ip无关
