...
import com.ylg.iot.media.zlm.hook.params.*;
...
/**
 * ZLMediaKit server的hook事件监听
 *
 * @author zjh
 * @since 2025-07-02 15:35
 */
@Slf4j
@RestController
...
public class ZLMHttpHookListener {
    ...
    public HookResult onServerStarted(HttpServletRequest request, @RequestBody JSONObject jsonObject) {
        ...
        try {
            ZlmServerStartEvent event = new ZlmServerStartEvent(this);
            MediaServer mediaServerItem = mediaServerService.getServerByServerId(zlmServerConfig.getMediaServerId());
    ...
    /**
     * zlm服务器心跳，上报间隔可配置，默认10s上报一次
     */
    @ResponseBody
    @PostMapping(value = "/on_server_keepalive", produces = "application/json;charset=UTF-8")
    public HookResult onServerKeepalive(@RequestBody OnServerKeepaliveHookParam param) {
        try {
            ZlmServerKeepAliveEvent event = new ZlmServerKeepAliveEvent(this);
            MediaServer mediaServerItem = mediaServerService.getServerByServerId(param.getMediaServerId());
            if (mediaServerItem != null) {
                event.setMediaServerItem(mediaServerItem);
                applicationEventPublisher.publishEvent(event);
            }
    ...
    public HookResult onPlay(@RequestBody OnPlayHookParam param) {
        ...
        if (mediaServer == null) {
            log.warn("[ZLM HOOK]播放鉴权 响应：{}->找不到对应的mediaServer", param.getMediaServerId());
            return HookResult.SUCCESS();
        }

        // 对于播放流进行鉴权
        AuthenticatePlayResult authenticateResult = mediaService.authenticatePlay(mediaServer, param.getApp(), param.getStream(), paramMap.get("callId"));
        if (!authenticateResult.isResult()) {
            log.info("[ZLM HOOK] 播放鉴权 失败：{}->{}", param.getMediaServerId(), param);
            return new HookResult(401, authenticateResult.getMessage());
        }
    ...
    /**
     * rtsp/rtmp/rtp 推流鉴权事件 TODO check 非级联平台是否会触发此事件
     */
    @ResponseBody
    ...
    public HookResult onStreamChanged(@RequestBody OnStreamChangedHookParam param) {
        MediaServer mediaServer = mediaServerService.getServerByServerId(param.getMediaServerId());
        if (mediaServer == null) {
            return HookResult.SUCCESS();
        }
        ...
        if (param.getSchema().equalsIgnoreCase("rtsp")) {
            if (param.isRegist()) {
                ...
                MediaArrivalEvent mediaArrivalEvent = MediaArrivalEvent.getInstance(this, param, mediaServer, serverInstanceConfig.getInstanceId());
    ...
    public JSONObject onStreamNoneReader(@RequestBody OnStreamNoneReaderHookParam param) {
        ...
        MediaServer mediaInfo = mediaServerService.getServerByServerId(param.getMediaServerId());
        if (mediaInfo == null) {
    ...
    public HookResult onHttpAccess(HttpServletRequest request, @RequestBody JSONObject jsonObject) {
    ...
    /**
     * 发送rtp(startSendRtp)被动关闭时回调 TODO check 非级联平台是否会触发此事件
     */
    @ResponseBody
